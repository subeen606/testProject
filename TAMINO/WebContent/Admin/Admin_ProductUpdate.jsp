...
<%
	...
	String sel_[] = {"의류","악세사리","앨범","기타"};
	System.out.println("update.jsp seq:"+dto.getSeq());
%>    
<!DOCTYPE html>
<html>
<head>
...
<p class="pageCaption">상품 수정</p>
<form action="/TAMINO/AdminCtrl?command=updateProduct" method="post" enctype="multipart/form-data">
	...
	<tr> 
		<th>상품명</th>
		<td colspan="2"><input type="text" name="title"  value="<%=dto.getProductName() %>"></td>
	</tr>
	<!-- 가격 -->
	<tr>
		<th>가격</th>
		<td colspan="2"><input type="text" name="price" value="<%=dto.getPrice()%>"></td>
	</tr>
	<!-- 상품분류 -->
	<tr>
		<th>분류</th>
		<td colspan="2">
			...
			<%
				for(int i=0; i<sel_.length; i++){
					if(sel_[i].equals(dto.getCode())){
						%><option selected="selected"><%=dto.getCode() %></option><%		
					}
					else{
						%><option><%=sel_[i] %></option><%
					}
				}
			%>
			</select>
	...
	<tr>
		...
		<%
			if(dto.getCode().equals("의류")){
				%>
				<td colspan="2">
					<input type="text" name="pCount_s" class="closeTd" placeholder="SIZE_S" value="<%=dto.getCount_s()%>" style="display: inline-block;">
					<input type="text" name="pCount_m" class="closeTd" placeholder="SIZE_M" value="<%=dto.getCount_m()%>" style="display: inline-block;">
					<input type="text" name="pCount_l" class="closeTd" placeholder="SIZE_L" value="<%=dto.getCount_l()%>" style="display: inline-block;">	
			
					<input type="text" name="pCount_o" class="otherTd" placeholder="SIZE_O" value="" style="display: none;">
				</td>
				<%			 
				}
			else{
				%>
				<td colspan="2">
					<input type="text" name="pCount_s" class="closeTd" placeholder="SIZE_S" value="" style="display: none;">
					<input type="text" name="pCount_m" class="closeTd" placeholder="SIZE_M" value="" style="display: none;">
					<input type="text" name="pCount_l" class="closeTd" placeholder="SIZE_L" value="" style="display: none;">	
					
					<input type="text" name="pCount_o" class="otherTd" placeholder="SIZE_O" value="<%=dto.getCount_o()%>" style="display: inline-block;">
				</td>
				<%
			}
		%> 
			...
			for(int i = 0; i < 2; i++){
				...
				<div class="filebox" style="width: 50%; height: auto; float: left;" >
					...
					if(pImg.split(",")[i].equals("")||pImg.split(",")[i]==null){
						...
						%>
						<img alt="" src="/TAMINO/product/<%=pImg.split(",")[i] %> "id="img<%=i+1%>">
						<%
					}
					%>	
				</div>
...
<script>
	...
	<tr>
		<td colspan="3">
			<p>
			<input type="submit" value="수정완료"  id="updateBtn" class="adminbtns" style="float: left;">
...
<form action="/TAMINO/AdminCtrl?command=updateProduct" method="post" enctype="multipart/form-data">
	...
	<tr>
		...
		<%
			if(dto.getCode().equals("의류")){
				%>
				<td colspan="3"  id="closeTd" style="display: block;">
					<input type="text" name="pCount_s"  value="<%=dto.getCount_s()%>">
					<input type="text" name="pCount_m"  value="<%=dto.getCount_m()%>">
					<input type="text" name="pCount_l"  value="<%=dto.getCount_l()%>">	
				</td>
				<td colspan="3"  id="otherTd" style="display: none;">
					<input type="text" name="pCount_o" value="<%=dto.getCount_o()%>">
				</td>
				<%			
				}
			else{
			...
			for(int i = 0; i < 2; i++){
				...
				<div class="filebox" style="width: 50%; height: auto; float: left;" >
					...
					if(pImg.split(",")[i].equals("")||pImg.split(",")[i]==null){
						...
						%>
						<img alt="" src="./product/<%=pImg.split(",")[i] %> "id="img<%=i+1%>">
						<%
					}
					%>	
				</div>
...
<script>
	...
	<tr>
		<td colspan="3">
			<p>
			<input type="submit" value="수정완료"  id="updateBtn" style="float: right;">
			<button type="button" style="float: left;" >뒤로가기</button>
			</p>	
		</td>
	</tr>
	</table>
