...
<div id="box1">
<p class="pageCaption" align="center">Schedule 수정</p>
<form id="scheduleUpdateform" action="/TAMINO/AdminCtrl?command=ScheduleUpdateAF" method="post" enctype="multipart/form-data">
...
<col width="100"><col width="500">
	<tr>
		<th>공연 명</th>
		<td>
			<input type="text" name="schedule_title" id="title_txt" 
				style="box-sizing: border-box; width: 90%" value="<%=dto.getSchedule_title() %>">
		</td>
	</tr>
		...
		<td>
			<select name="schedule_year" id="year_txt">
				<%for(int i=year; i<year+5; i++){ 
					if(i == Integer.parseInt(dto.getSchedule_date().substring(0, 4))){%>
			...
			<select name="schedule_month" id="month_txt">
				<%for(int i=1; i<=12; i++){ 
					if(i == Integer.parseInt(dto.getSchedule_date().substring(4, 6))){%>
			...
			<select name="schedule_date" id="date_txt">
				<%for(int i=1; i<=31; i++){
					if(i == Integer.parseInt(dto.getSchedule_date().substring(6, 8))){%>
	...
	<tr>
		...
		<td>
			<select name="schedule_hour" id="hour_txt">
				<%for(int i=0; i<24; i++){
					if(i == Integer.parseInt(dto.getSchedule_time().substring(0, 2))){%>
			...
			<select name="schedule_minute" id="minute_txt">
				<%for(int i=0; i<=50; i+=10){
					if(i == Integer.parseInt(dto.getSchedule_time().substring(2, 4))){%>
	...
	<tr>
		<th>예매처</th>
		<td>
			<input type="text" name="ticketlink" id="ticket_txt" 
				style="box-sizing: border-box; width: 90%" value="<%=dto.getTicketLink() %>">
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<input type="hidden" name="old_poster_file" value="<%=dto.getSchedule_poster() %>">
		</td>
	</tr>
	...
	<tr>
		<th>교체 사진</th>
		<td>
			<input type="file" name="new_poster_file" >
		</td>
	</tr>
	...
	<tr>
		<td colspan="2" align="center">
			<textarea rows="10" style="width: 95%" name="schedule_content" id="content_txt" ><%=dto.getSchedule_content() %></textarea>
		</td>
	</tr>
	...
	<tr>	
		<td colspan="2" align="center">
			...
			<input type="text" id="address_txt" name="schedule_place" style="height: 25px;" placeholder="주소" value="<%=dto.getSchedule_place() %>">
			<input type="button" class="adminbtns" id="addressBtn" onclick="sample3_execDaumPostcode()" value="주소 찾기"><br>
	...
	<tr>
		<td colspan="2" align="center">
			<input type="submit" class="adminbtns" id="updateBtn" value="수정하기">
		</td>
	</tr>
...
<col width="100"><col width="500">
	...
	<tr>	
		<td colspan="2" align="center">
			...
			<input type="text" id="address_txt" name="schedule_place" placeholder="주소" value="<%=dto.getSchedule_place() %>">
...
$(function () {
	...
	$("#addressBtn").mouseover(function() {		
		$(this).css("background", "#AAABD3");
	}); 
	$("#addressBtn").mouseout(function() {		
