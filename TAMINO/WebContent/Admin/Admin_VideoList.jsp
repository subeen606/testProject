...
tr.detail:hover { background-color: lightyellow; } 
...
<form id="frm" action="/TAMINO/AdminCtrl" method="post">
<input type="hidden" name="command" value="videodelete">

<div id="box1" >
<p class="pageCaption">Video 목록</p>
<div id="btndiv">
<input id="btn1" type="submit" class="adminbtns" value="Video추가">
</div>
<br>
...
<thead>
<tr class="title">
	<th width="5%%"><div class="th-text"><input type="checkbox" name="alldel" onclick="deletechecks(this.checked)"></div></th>
		<th width="15%"><div class="th-text">번호</th><th width="35%"><div class="th-text">제목</div></th><th width="45%"><div class="th-text">video</div></th>
</tr>
</thead>
<%
if(list == null || list.size() == 0){
	...
	<tbody>	
	<tr class="detail">	
	<td style="padding-left: 30px;"><input type="checkbox" name="delck" value="<%=dto.getNews_seq() %>"></td>	
		<th><%=i + 1 %></th>
		<td>
			<% 
			if(dto.getDel() == 0){
		...
		<td align="center"><p class="videosize"><%=dto.getContent() %></p></td>	
	</tr>
	</tbody>	
	<%	
	}	
...
<input type="submit" class="adminbtns" value="Video삭제" >
</div>
...
$("#btn1").click(function () {
	$("#frm").attr("action", "/TAMINO/AdminCtrl?command=addvideo").submit();
});
