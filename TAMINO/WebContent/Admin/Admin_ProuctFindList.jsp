...
%>
	 <tr>
	 	<th colspan="6">'<%=fText%>'의 검색결과가 없습니다.<br>검색어를 확인해 주세요!
	 	</th>
	 </tr>
	 <%
	 	}
	 ...
	 %>
		 <tr class="detail">
		 	...
		 	<th id="_1"><%=dto.getSeq() %></th>
		 	<th id="_2"><a href="/TAMINO/AdminCtrl?command=detail&seq=<%=dto.getSeq()%>"><%=dto.getProductName() %></a></th>
		 	<th id="_3"><%=dto.getCode() %></th>
		 	<th id="_4"><%=dto.getPrice() %></th>
		 	<%
		 	if(dto.getCount_o()==0){
		 		%>
		 		<th id="_5">
...
<tr>
	<th colspan="6" style="border: none">
		...
		for(int i=1; i<pageNum+1; i++){
			%>
			<a href="/TAMINO/AdminCtrl?command=finding&sel=<%=sel%>&fText=<%=fText%>&page=<%=i-1%>>"><%=i %></a>
			<%
		}
		%>
	</th>
...
%>
	 <tr>
	 	<th colspan="6">'<%=fText%>'의 검색결과가 없습니다.<br>
	 	검색어를 확인해 주세요!
	 	</th>
	 </tr>
	 <%
...
<tr> <!-- 페이징 -->
	<th colspan="6" style="border: none">
		...
		for(int i=1; i<pageNum+1; i++){
			%>
			<a href="/TAMINO/AdminCtrl?command=finding&select=<%=sel%>&fText=<%=fText%>&page=<%=i-1%>"><%=i %></a>
			<%
		}
		%>
	</th>
