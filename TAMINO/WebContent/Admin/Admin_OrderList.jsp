...
<%
	...
	/* System.out.println("memberList list.size():"+list.size()); */
%>
<!DOCTYPE html>
<html>
<head>
...
<p class="pageCaption" align="left">주문 목록</p>
 ...
 if(list==null || list.size()==0){
	 ...
	 for(OrderListDto dto : list){
		 ...
		 <tr class="detail" seq="<%=dto.getOderlist_seq() %>">
		 	<td align="center"><%=dto.getOderlist_seq() %></td>
		 	<td align="center"><%=dto.getOrder_email() %></td>
		 	<td align="center"><%=dto.getOrder_phonenumber() %></td>
		 	<td align="center"><%=dto.getProduct_name() %></td>
		 	<td align="center"><%=dto.getProduct_count() %></td>
		 	<td align="center"><%=dto.getProduct_count()*dto.getProduct_price() %></td>
		 </tr>
		 <%
		 }
	 }
...
<tr>
	<th colspan="6" style="border: none;">
		<select id="_findSel">
			<option selected="selected" value="이메일">이메일</option>
...
<tr>
	<th colspan="6" style="border: none">
		...
		for(int i=1; i<pageNum+1; i++){
			%>
			<a style="text-decoration: none; color: black" href="/TAMINO/AdminCtrl?command=order_list&page=<%=i-1%>"><%=i %></a>
			<%
		}
		%>
	</th>
